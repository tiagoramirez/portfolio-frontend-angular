...
export class NewSocialMediaComponent implements OnInit {
    ...
    constructor(private route: ActivatedRoute, private router: Router, private tokenService: TokenService, private socialMediaService: SocialMediaService) { }

    ngOnInit(): void {
        this.username = this.route.snapshot.params['username'];
        let subSocialMedia = this.socialMediaService.getAll().subscribe({
            next: (data) => {
    ...
    onSave() {
        ...
        let sub = this.socialMediaService.addNew(this.user_social_media).subscribe({
            error: (err) => {
                if (err.error.messageControlled !== undefined && err.error.messageControlled == true) {
