...
export class EditPhotoComponent implements OnInit, OnDestroy {
    ...
    constructor(private route: ActivatedRoute, private photoService: PhotoService, private router: Router, private bannerService: BannerService, private tokenService: TokenService) { }
    ...
    saveImg() {
        if (this.photoFile !== undefined && this.photoFile !== null) {
            if (this.type === 'photo' && this.isFirstPhoto) {
                let subPhoto: Subscription = this.photoService.addNew(this.photoFile, this.tokenService.getUserId()).subscribe({
                    next: () => {
                ...
                let subBanner: Subscription = this.bannerService.addNew(this.photoFile, this.tokenService.getUserId()).subscribe({
                    next: () => {
                        console.log("Nueva foto cargada exitosamente");
                    },
                    error: (e) => {
                ...
                let subPhoto: Subscription = this.photoService.edit(this.photoFile, this.tokenService.getUserId(), this.photoId).subscribe({
                    next: () => {
                        console.log("Nueva foto cargada exitosamente");
                    },
                    error: (e) => {
                ...
                let subBanner: Subscription = this.bannerService.edit(this.photoFile, this.tokenService.getUserId(), this.photoId).subscribe({
                    next: () => {
                        console.log("Nueva foto cargada exitosamente");
                    },
                    error: (e) => {
